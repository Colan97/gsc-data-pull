...
try:
    ...
    st.error("OAuth credentials (GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, REDIRECT_URI) not found in st.secrets. Please configure them in .streamlit/secrets.toml")
    st.stop()
...
def login_clicked():
    ...
    # st.markdown(f'Please [click here to authorize]({authorization_url}) if you are not redirected automatically.', unsafe_allow_html=True)
    ...
    if 'keys' in df.columns:
        for i, dim_name in enumerate(requested_dimensions):
            try:
                ...
                df[dim_name] = None
        # df.drop(columns=['keys'], inplace=True, errors='ignore') # No longer needed
...
if 'code' in query_params and 'state' in query_params:
    if 'oauth_state' not in st.session_state or st.session_state['oauth_state'] != query_params.get('state', [None])[0]:
...
if st.sidebar.button("🚀 Fetch & Analyze MoM Data", type="primary"):
        ...
        if df_current_processed.empty and df_previous_processed.empty:
            ...
            if not mom_df_for_period.empty:
                ...
                for metric_name in metrics_for_display:
                    if metric_name in mom_df_for_period.columns: display_cols_order.append(metric_name)
                    if f'prev_{metric_name}' in mom_df_for_period.columns: display_cols_order.append(f'prev_{metric_name}')
                    if f'{metric_name}_mom' in mom_df_for_period.columns: display_cols_order.append(f'{metric_name}_mom')
    ...
    if all_mom_dataframes:
        final_report_df = pd.concat(all_mom_dataframes).sort_values(by=['month_year'] + (['page'] if report_level == 'page-level' and 'page' in pd.concat(all_mom_dataframes).columns else []), ascending=[False] + ([True] if report_level == 'page-level' and 'page' in pd.concat(all_mom_dataframes).columns else [])).reset_index(drop=True)
    ...
    elif not st.session_state.get('error_occurred', False): # Check if errors were not already shown
